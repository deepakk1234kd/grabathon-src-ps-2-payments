...
const firestore = new Firestore({
  projectId: 'machine-learning-257016',
        ...
        return new Promise((resolve, reject) => {
            ...
            firestore.collection(USERS_COLLECTION).where('phone-number', '==' , sourcePhoneNumber).get().then(snapshot => {
                ...
                snapshot.forEach(doc => {
                    ...
                    const user = doc.data();
                    if(!user['is-authenticated']) {
                        const otpMessage = 'You need to be authenticated to perform this operation <break time="0.5s"/> We have sent an OTP to your phone number <say-as interpret-as="telephone">' + sourcePhoneNumber + '</say-as> please mention it';
                        res.json({
                            "fulfillmentMessages": [{ 
                            "platform": "GOOGLE_TELEPHONY", 
                            "telephonySynthesizeSpeech": { "ssml": getFullMessage(otpMessage) }
        ...
        return new Promise((resolve,reject) => {
            firestore.collection(USERS_COLLECTION).where('phone-number', '==' , sourcePhoneNumber).get().then(snapshot => {
                ...
                snapshot.forEach(doc => {
                    console.log(doc.id, '=>', doc.data());
                    const user = doc.data();
                    if(user.otp !== responseBody.queryResult.parameters['otp']) {
                        res.json({
                            "fulfillmentMessages": [{ 
                            "platform": "GOOGLE_TELEPHONY", 
